...
public class PickUpLetters : MonoBehaviour {
    ...
    {
        if (_pickups.Contains(other.name) && this.gameObject.transform.root.name == GameObject.FindObjectOfType<PowerUpScriptP1>().transform.root.name)
        {
            if (_pickups.Contains(this.gameObject.name))
            {
                GameObject.FindObjectOfType<Player1MoveScript>().GetComponent<PowerUpScriptP1>().PickedUpP1.Add(this.gameObject.name);
                _pickups.Remove(this.gameObject.name);
                if (_pickups.Count == 0)
                {
        ...
        if (_pickups.Contains(other.name) && this.gameObject.transform.root.name == GameObject.FindObjectOfType<PowerUpScriptP2>().transform.root.name)
        {
            if (_pickups.Contains(this.gameObject.name))
            {
